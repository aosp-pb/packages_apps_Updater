...
updates_dir=/data/aospb_updates

if [ ! -f "$1" ]; then
   echo "Usage: $0 ZIP [UNVERIFIED]"
   echo "Push ZIP to $updates_dir and add it to Updater"
   echo
   echo "The name of ZIP is assumed to have aospb-VERSION-DATE-TYPE-* as format"
...
# Assume aospb-VERSION-DATE-TYPE-*.zip
...
adb shell "killall org.aospb.updater 2>/dev/null"
adb shell "sqlite3 /data/data/org.aospb.updater/databases/updates.db" \
